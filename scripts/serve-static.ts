...
const defaultRootPath = resolve(dirname(fileURLToPath(import.meta.url)), '..');
...
export async function createStaticServer(
  ...
  const openReadStream = options.openReadStream ?? createReadStream;
  ...
  return createServer(async (request, response) => {
    ...
    try {
      ...
      const stream = openReadStream(realFilePath);
      stream.once('error', () => {
        response.destroy();
      });
...
if (
  ...
  const server = await createStaticServer({ rootPath: defaultRootPath });
