...
这份手册给出组件的完整公共接口、CSS 变量、头像网络行为和数据维护命令。面向需要逐项核对接口或做数据维护的开发者。想直接把组件嵌入已有小游戏，按步走请看 [`embed-in-existing-game.md`](./embed-in-existing-game.md)；想知道项目是什么，请看根目录 `README.md`。
